...
export function writeU8(buf: Uint8Array, offset: number, value: number): number {
  buf[offset] = value;
  return offset + 1;
}

export function writeU64(buf: Uint8Array, offset: number, value: bigint): number {
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  view.setBigUint64(offset, value, true);
  return offset + 8;
}

export function writeI64(buf: Uint8Array, offset: number, value: bigint): number {
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  view.setBigInt64(offset, value, true);
  return offset + 8;
}

export function writeU16(buf: Uint8Array, offset: number, value: number): number {
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
...
export function ixData(bytes: Uint8Array): Buffer {
