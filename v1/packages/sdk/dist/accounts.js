...
export function decodeMarket(data, pubkey) {
    if (data.length < 197)
        return null;
    let o = 0;
    const magic = readU64(data, o);
    o += 8;
    if (magic !== MARKET_MAGIC)
        return null;
...
export function decodePosition(data, pubkey) {
    if (data.length < 91)
        return null;
    let o = 0;
    const magic = readU64(data, o);
...
export async function fetchMarket(connection, address) {
    const info = await connection.getAccountInfo(address);
    if (!info?.data)
        return null;
    return decodeMarket(new Uint8Array(info.data), address);
}
export async function fetchPosition(connection, address) {
    const info = await connection.getAccountInfo(address);
    if (!info?.data)
        return null;
